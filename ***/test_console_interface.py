...
class TestUserInterface:
    ...
    def unified_mock_environment(self):
        ...
        with patch("builtins.input", return_value="0"), patch("builtins.print"), patch("requests.get"), patch(
            "requests.post"
        ), patch("psycopg2.connect", return_value=mock_connection), patch("os.path.exists", return_value=True), patch(
            "os.makedirs"
        ), patch(
            "src.utils.env_loader.EnvLoader.load_env_file"
        ), patch(
            "src.storage.postgres_saver.PostgresSaver", return_value=mock_storage
        ), patch(
            "src.storage.db_manager.DBManager", return_value=mock_db_manager
        ), patch(
            "src.api_modules.unified_api.UnifiedAPI", return_value=mock_unified_api
        ), patch(
            "src.ui_interfaces.vacancy_search_handler.VacancySearchHandler", return_value=mock_search_handler
        ), patch(
            "src.ui_interfaces.vacancy_display_handler.VacancyDisplayHandler", return_value=mock_display_handler
        ), patch(
            "src.ui_interfaces.vacancy_operations_coordinator.VacancyOperationsCoordinator",
            return_value=mock_operations_coordinator,
        ), patch(
            "src.utils.vacancy_operations.VacancyOperations", return_value=mock_vacancy_ops
        ), patch(
            "src.utils.db_manager_demo.DBManagerDemo", return_value=mock_demo
        ), patch(
            "src.storage.storage_factory.StorageFactory.get_default_storage", return_value=mock_storage
        ):
    ...
    def test_advanced_search_vacancies_with_keywords(self, unified_mock_environment, sample_vacancy):
        ...
        with patch("src.utils.ui_helpers.get_user_input", return_value="python, django"), patch(
            "src.utils.ui_navigation.quick_paginate"
        ) as mock_paginate:
    ...
    def test_advanced_search_vacancies_with_operators(self, unified_mock_environment, sample_vacancy):
        ...
        with patch("src.utils.ui_helpers.get_user_input", return_value="python AND django"), patch(
            "src.utils.ui_navigation.quick_paginate"
        ) as mock_paginate:
    ...
    def test_filter_by_min_salary(self, unified_mock_environment, sample_vacancy):
        ...
        with patch("builtins.input", side_effect=["1", "100000"]), patch(
            "src.utils.ui_navigation.quick_paginate"
        ) as mock_paginate:
    ...
    def test_filter_by_max_salary(self, unified_mock_environment, sample_vacancy):
        ...
        with patch("builtins.input", side_effect=["2", "200000"]), patch(
            "src.utils.ui_navigation.quick_paginate"
        ) as mock_paginate:
    ...
    def test_filter_by_salary_range(self, unified_mock_environment, sample_vacancy):
        ...
        with patch("builtins.input", side_effect=["3", "100000 - 200000"]), patch(
            "src.utils.ui_helpers.parse_salary_range", return_value=(100000, 200000)
        ), patch("src.utils.ui_navigation.quick_paginate") as mock_paginate:

            interface._filter_saved_vacancies_by_salary()

            mocks["storage"].get_vacancies.assert_called_once()
            mocks["vacancy_ops"].filter_vacancies_by_salary_range.assert_called_once_with(
                [sample_vacancy], 100000, 200000
            )
            mocks["vacancy_ops"].sort_vacancies_by_salary.assert_called_once()
            mock_paginate.assert_called_once()
    ...
    def test_show_vacancies_for_deletion_single(self, unified_mock_environment, sample_vacancy):
        ...
        with patch("builtins.input", side_effect=["1", "y"]), patch(
            "src.utils.ui_helpers.confirm_action", return_value=True
        ):
    ...
    def test_show_vacancies_for_deletion_all(self, unified_mock_environment, sample_vacancy):
        ...
        with patch("builtins.input", side_effect=["a"]), patch(
            "src.utils.ui_helpers.confirm_action", return_value=True
        ):
    ...
    def test_run_method_navigation(self, unified_mock_environment):
        ...
        with patch.object(interface, "_show_menu", side_effect=["1", "0"]), patch.object(
            interface, "_search_vacancies"
        ) as mock_search:
