...
class TestSourceManager:
    ...
    def test_get_vacancies_from_all_sources(self, source_manager):
        ...
        mock_source1 = Mock()
        mock_source1.get_vacancies.return_value = [
            {"id": "1", "title": "Job 1", "source": "source1"}
        ]

        mock_source2 = Mock()
        mock_source2.get_vacancies.return_value = [
            {"id": "2", "title": "Job 2", "source": "source2"}
        ]
    ...
    def test_source_manager_methods_exist(self, source_manager):
        ...
        expected_methods = [
            'get_vacancies_from_all_sources', 'get_vacancies_from_source',
            'register_source', 'activate_source', 'deactivate_source',
            'get_source_stats', 'clear_active_sources'
        ]

        existing_methods = [method for method in expected_methods
                          if hasattr(source_manager, method)]
    ...
    def test_source_manager_integration(self, source_manager):
        ...
        methods = [attr for attr in dir(source_manager)
