...
class TestSingleSQLFiltering:
    ...
    def test_single_sql_filtering_execution(self, mock_db_connection: Dict[str, Any], sample_vacancies: List[Vacancy]) -> None:
        ...
        with patch('src.storage.postgres_saver.PostgresSaver.get_connection') as mock_get_conn:
            ...
            create_temp_calls = [call for call in cursor.execute.call_args_list 
    ...
    def test_filtering_only_in_postgres_saver(self) -> None:
        ...
        filtering_methods = [attr for attr in dir(unified_api) if 'filter' in attr.lower() and 'companies' in attr.lower()]
        assert len(filtering_methods) == 0, f"UnifiedAPI не должен содержать методы фильтрации по компаниям: {filtering_methods}"
        ...
        with patch('src.storage.postgres_saver.PostgresSaver.get_connection') as mock_get_conn:
            ...
            with patch.object(postgres_saver, 'filter_and_deduplicate_vacancies', return_value=[vacancy_with_fields]):
                ...
                insert_calls = [call for call in cursor.execute.call_args_list 
                               if "INSERT INTO temp_new_vacancies" in str(call)]
                
                assert len(insert_calls) >= 1, "Должна быть выполнена вставка с полями description, requirements, responsibilities"
