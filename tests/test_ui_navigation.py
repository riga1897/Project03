...
class TestUINavigation:
    ...
    def test_handle_menu_choice_valid(self, mock_input, mock_print, nav):
        ...
        items = [{"text": "Пункт 1", "action": lambda: "action1"}, {"text": "Пункт 2", "action": lambda: "action2"}]
    ...
    def test_unicode_support(self, mock_input, mock_print, nav):
        ...
        items = [{"text": "🔍 Поиск"}, {"text": "⚙️ Настройки"}, {"text": "📊 Статистика"}]
    ...
    def test_special_characters_in_menu(self, mock_input, mock_print, nav):
        ...
        items = [{"text": "Пункт с &*#@!"}, {"text": "Пункт с 'кавычками'"}, {"text": 'Пункт с "двойными кавычками"'}]
