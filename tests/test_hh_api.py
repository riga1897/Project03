...
class TestHeadHunterAPI:
    ...
    @patch('src.api_modules.hh_api.logger')
    def test_get_vacancies_from_target_companies(self, mock_logger, hh_api):
        ...
        with patch('src.config.target_companies.TargetCompanies') as mock_target_companies:
            mock_target_companies.return_value.get_hh_ids.return_value = ["company1", "company2"]
    ...
    def test_clear_cache(self, hh_api):
        """Тест очистки кэша"""
        # Мокаем суперклассы и их методы, чтобы избежать реальных операций
        mock_super_instance = Mock()
        # Предполагаем, что у CachedAPI есть метод clear_cache, который принимает 'source'
        # и мы хотим проверить, что он вызывается с правильным аргументом 'hh'
        with patch.object(hh_api, 'clear_cache') as mock_super_clear_cache:
             # Call the method that internally calls super().clear_cache()
             hh_api.clear_cache("hh")
             # Assert that the mocked super method was called correctly
             mock_super_clear_cache.assert_called_once_with("hh")
