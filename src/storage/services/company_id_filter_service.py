...
class CompanyIDFilterService(AbstractFilterService):
    ...
    def _execute_id_filter_query(self, cursor) -> List[str]:
        ...
        # Фильтрация по SJ ID
        if sj_ids_list:
            query_parts.append(f"((source LIKE '%sj%' OR source LIKE '%superjob%') AND employer_id IN ({sj_ids_list}))")
        
        # Если источник неизвестен, проверяем оба списка
        if hh_ids_list and sj_ids_list:
            query_parts.append(f"(source NOT LIKE '%hh%' AND source NOT LIKE '%sj%' AND source NOT LIKE '%superjob%' AND (employer_id IN ({hh_ids_list}) OR employer_id IN ({sj_ids_list})))")
        elif hh_ids_list:
            query_parts.append(f"(source NOT LIKE '%hh%' AND source NOT LIKE '%sj%' AND source NOT LIKE '%superjob%' AND employer_id IN ({hh_ids_list}))")
        elif sj_ids_list:
            query_parts.append(f"(source NOT LIKE '%hh%' AND source NOT LIKE '%sj%' AND source NOT LIKE '%superjob%' AND employer_id IN ({sj_ids_list}))")
        ...
        if not filtered_ids:
            ...
            cursor.execute("SELECT DISTINCT employer_id, source FROM temp_company_filter WHERE employer_id IS NOT NULL LIMIT 10")
