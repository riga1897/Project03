...
class VacancyStorageService(AbstractVacancyStorageService):
    ...
    def add_vacancy_batch_optimized(self, vacancies: List[Vacancy], search_query: str = None) -> List[str]:
        ...
        if len(vacancies) == 1 and isinstance(vacancies[0], list):
            ...
            logger.debug(f"VacancyStorageService: исправлена двойная вложенность списка: получено {len(vacancies)} вакансий")
    ...
    def _prepare_vacancy_data(self, vacancy: Vacancy, company_mapping: Dict[str, int], search_query: str = None) -> Optional[tuple]:
    ...
    def load_vacancies(self, limit: Optional[int] = None, offset: int = 0, 
                      filters: Optional[Dict[str, Any]] = None) -> List[Vacancy]:
        """
            ...
            if salary_info and salary_info != 'Не указана':
                ...
                if salary_match:
                    if len(salary_match) >= 2:
                        salary_data = {
                            "from": int(salary_match[0]),
                            "to": int(salary_match[1]),
                            "currency": "RUR"
                        }
