...
class SQLDeduplicationService(AbstractDeduplicationService):
    ...
    def _execute_deduplication_query(self, cursor) -> List[str]:
        ...
        duplicates_count = len([row for row in cursor.execute("SELECT COUNT(*) FROM temp_deduplication").fetchall()[0]]) - len(unique_ids) if cursor.rowcount else 0
        logger.info(f"SQL-дедупликация: найдено {len(unique_ids)} уникальных вакансий, удалено {duplicates_count} дубликатов")
